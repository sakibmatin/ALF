...
def train_HIPNN_model(model_dir,
    ...
    with hippynn.tools.active_directory(model_dir):
        with hippynn.tools.log_terminal("training_log.txt","wt"):
            ...
            if electrostatics_flag is None: # Just the energy Nodes, i.e. standard HIPNN. 
            ...
            if not(electrostatics_flag is None):
                if not(charges_key is None):
            ...
            if not(quadrupole_key is None) and not(electrostatics_flag is None):
...
def HIPNN_ASE_calculator(HIPNN_model_directory, device="cuda:0"):
    ...
    try: # Try to load charge node if present. 
        charge_node = model.node_from_name("HCharge")
        calc = HippynnCalculator(energy_node, charge_node, en_unit=units.eV)
    except: # Only energy node. 
